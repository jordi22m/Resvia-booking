-- =============================================================================
-- Fix API availability endpoint: compute real free slots
-- =============================================================================
...
BEGIN
  SELECT p.id, p.user_id, p.slug, p.business_name,
         COALESCE(p.min_notice_minutes, 0) AS min_notice_minutes,
         COALESCE(p.max_days_ahead, 60) AS max_days_ahead,
         COALESCE(p.allow_weekends, true) AS allow_weekends
  INTO v_profile
  ...
  v_min_notice_minutes := COALESCE(v_profile.min_notice_minutes, 0);
  v_max_days_ahead := COALESCE(v_profile.max_days_ahead, 60);
  v_allow_weekends := COALESCE(v_profile.allow_weekends, true);
  ...
  IF p_service_id IS NOT NULL THEN
    ...
    WHERE s.id = p_service_id
      AND s.user_id = v_profile.user_id
  ...
  v_duration_minutes := GREATEST(COALESCE(v_service.duration, 30), 5);
  v_slot_step_minutes := GREATEST(
    COALESCE(NULLIF(p_slot_minutes, 0), NULLIF(v_service.slot_step_minutes, 0), 30),
    5
  );
  v_requires_staff := COALESCE(v_service.requires_staff, true);
  ...
  ELSIF v_requested_staff_name IS NOT NULL THEN
    ...
    INTO v_resolved_staff
    FROM public.staff_members st
    WHERE st.user_id = v_profile.user_id
      AND COALESCE(st.active, true) = true
      AND (
  ...
  IF (p_staff_id IS NOT NULL OR v_requested_staff_name IS NOT NULL) AND v_resolved_staff.id IS NULL THEN
    RAISE EXCEPTION 'Profesional no encontrado para este workspace';
  END IF;

  IF v_resolved_staff.id IS NOT NULL AND v_has_service_staff_mapping THEN
    IF NOT EXISTS (
      SELECT 1
      FROM public.staff_services ss
      WHERE ss.service_id = v_service.id
        AND ss.staff_id = v_resolved_staff.id
    ) THEN
  ...
  LOOP
    ...
    IF COALESCE(v_exception.is_closed, false) = true THEN
      CONTINUE;
    END IF;
    ...
    FOR v_staff_candidate IN
      ...
      FROM (
        ...
        WHERE st.user_id = v_profile.user_id
          AND COALESCE(st.active, true) = true
          AND (v_resolved_staff.id IS NULL OR st.id = v_resolved_staff.id)
          AND (
        ...
        WHERE v_requires_staff = false AND v_resolved_staff.id IS NULL
      ) c
    LOOP
      IF v_staff_candidate.staff_id IS NOT NULL THEN
        SELECT EXISTS (
          ...
          WHERE a.user_id = v_profile.user_id
      ...
      FOR v_window IN
        ...
        FROM (
          SELECT v_exception.start_time AS start_time, v_exception.end_time AS end_time
          WHERE COALESCE(v_exception.is_closed, false) = false
            AND v_exception.start_time IS NOT NULL
          ...
          WHERE a.user_id = v_profile.user_id
            ...
            AND NOT (
              COALESCE(v_exception.is_closed, false) = false
              AND v_exception.start_time IS NOT NULL
              AND v_exception.end_time IS NOT NULL
            )
        ) q
      ...
      LOOP
        ...
        WHILE v_cursor <= v_latest_start LOOP
          ...
          WHERE ap.user_id = v_profile.user_id
            AND ap.date = v_day
          ...
          SELECT EXISTS (
            ...
            WHERE cb.business_id = v_profile.user_id
  ...
  RETURN jsonb_build_object(
    ...
    'resolved_staff', CASE
      WHEN v_resolved_staff.id IS NULL THEN NULL
      ELSE jsonb_build_object(
        'id', v_resolved_staff.id,
        'name', v_resolved_staff.name
      )
    END,
