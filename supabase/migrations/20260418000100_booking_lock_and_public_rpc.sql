-- Prevent double-bookings under concurrency and expose a safe booking RPC.
...
BEGIN
  IF NOT EXISTS (
    ...
    EXCEPTION
      WHEN others THEN
        RAISE NOTICE 'Could not create appointments_no_overlap_per_staff: %', SQLERRM;
    END;
  END IF;
END
$$;
...
BEGIN
  ...
  -- Ensure the service belongs to the target business and is bookable.
  IF NOT EXISTS (
    SELECT 1
  ...
  -- Check slot is inside at least one active availability window.
  ...
  -- Final conflict check just before insertion.
  SELECT EXISTS (
    SELECT 1
