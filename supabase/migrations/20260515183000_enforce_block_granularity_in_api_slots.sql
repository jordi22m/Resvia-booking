-- =============================================================================
-- Enforce block-level 30min granularity in API availability slots
-- Mirrors frontend slot-generation rule used in booking-utils.ts
-- =============================================================================
    ...
    LOOP
      ...
      LOOP
        ...
        FROM (
          SELECT
            MOD(
              (EXTRACT(HOUR FROM ap30.end_time_resolved)::int * 60)
              + EXTRACT(MINUTE FROM ap30.end_time_resolved)::int,
              60
            ) AS phase,
            COUNT(*) AS cnt
          FROM (
            SELECT
              ap.start_time,
              COALESCE(ap.end_time, (ap.start_time + interval '30 minutes')::time) AS end_time_resolved,
              (
                EXTRACT(
                  EPOCH FROM (
                    (ap.date + COALESCE(ap.end_time, (ap.start_time + interval '30 minutes')::time))
                    - (ap.date + ap.start_time)
                  )
                ) / 60
          ...
          WHERE ap30.duration_minutes = 30
            AND ap30.end_time_resolved > v_window.start_time
            AND ap30.start_time < v_window.end_time
          GROUP BY phase
          ORDER BY cnt DESC, phase ASC
          LIMIT 1
        ) ranked;
        ...
        WHILE v_cursor <= v_latest_start LOOP
          ...
          IF v_forced_phase IS NOT NULL THEN
            v_slot_start_minutes :=
              (EXTRACT(HOUR FROM v_slot_start)::int * 60)
              + EXTRACT(MINUTE FROM v_slot_start)::int;

            IF MOD((v_slot_start_minutes - v_forced_phase + 60), 60) <> 0 THEN
              v_cursor := v_cursor + make_interval(mins => v_slot_step_minutes);
              CONTINUE;
            END IF;
          END IF;
